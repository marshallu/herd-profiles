<?php
/**
 * Template part for displaying profiles as an Icon Card.
 *
...
if ( 2 === intval( get_field( 'department_per_row', $the_term ) ) ) {
	$grid = 'herd:lg:grid-cols-2';
} elseif ( 3 === intval( get_field( 'department_per_row', $the_term ) ) ) {
	$grid = 'herd:lg:grid-cols-3';
} else {
	$grid = 'herd:lg:grid-cols-4';
}
?>
<div class="herd:grid herd:grid-cols-1 herd:md:grid-cols-2 <?php echo esc_attr( $grid ); ?> herd:gap-8">
<?php
while ( have_posts() ) {
	the_post();
		...
		<div class="herd:h-full herd:flex herd:flex-col herd:pb-8 herd:px-6">
			...
			<div class="herd:my-2 herd:flex-1 herd:border-t herd:border-gray-100 herd:mt-6 herd:pt-6 herd:text-gray-600 herd:font-normal">
				...
				<?php if ( get_field( 'employee_email_address' ) && ( 'both' === get_field( 'profile_show_email_address', 'option' ) || 'listing' === get_field( 'profile_show_email_address', 'option' ) ) ) { ?>
					<div class="herd:my-2">Email:&nbsp;<a href="mailto:<?php echo esc_attr( get_field( 'employee_email_address' ) ); ?>"><?php echo esc_html( get_field( 'employee_email_address' ) ); ?></a></div>
				<?php } ?>
			</div>
		</div>
	</div>
